...
from utilities.data_visualisation import process_cities, find_size, find_clusters, find_timings, find_temp_alt_correlation
...
def index():
    ...
    if request.method == 'GET':
        ...
        af = ["Cape Town, South Africa", "Kinshasa, Democratic Republic of the Congo", "Luanda, Angola", "Nairobi, Kenya"]
        an = ["McMurdo Station"]
        aa = ["Karachi, Pakistan", "Dhaka, Bangladesh", "Delhi, India", "Mumbai, India"]
        au = ["Sydney, Australia", "Melbourne, Australia", "Brisbane, Australia", "Perth, Australia"]
        eu = ["Moscow, Russia", "Paris, France", "London, United Kingdom", "Madrid, Spain"]
        na = ["Mexico City, Mexico", "New York City, United States", "Los Angeles, United States", "Chicago, United States"]
        sa = ["Rio de Janeiro, Brazil", "Santiago, Brazil", "Quito, Ecuador", "Cali, Colombia"]
        ...
        if continent_count == 0:
            chosen_cities = ["Cape Town, South Africa",
        ...
        resultDictSizeClusterTimeElevation = find_temp_alt_correlation(resultDictSizeClusterTime, app.config["DB_NAME"],
                                                 app.config["DB_PASSWORD"], app.config["DB_USER"],
                                                 app.config["DB_HOST"])

        return render_template('index.html', af=af, an=an, aa=aa, au=au, eu=eu, na=na, sa=sa,
                               result=resultDictSizeClusterTimeElevation)
